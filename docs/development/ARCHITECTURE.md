...
This document describes the technical architecture of the GOVJE Parking custom component for Home Assistant.
...
│   ├── __init__.py          # Exports GOVJEParkingDataUpdateCoordinator
...
│   ├── __init__.py          # Exports GOVJEParkingEntity
...
**Key class:** `GOVJEParkingDataUpdateCoordinator` (exported from `coordinator/__init__.py`)
...
**Key class:** `GOVJEParkingApiClient`
...
- `GOVJEParkingConfigFlowHandler` (main flow)
- `GOVJEParkingOptionsFlow` (options)
...
**Key class:** `GOVJEParkingEntity` (in `entity/base.py`)
...
2. `GOVJEParkingEntity` for common functionality
...
1. Create directory: `custom_components/govje_parking/<platform>/`
2. Implement `__init__.py` with `async_setup_entry()`
3. Create entity classes inheriting from platform base + `GOVJEParkingEntity`
