...
- **Title:** GOVJE Parking
- **Repository:** Migz93/ha-govje_parking

**Key directories:**

- `custom_components/govje_parking/` - Main integration code
...
- Integration logs: `custom_components.govje_parking: debug` in `config/configuration.yaml`
...
- **Domain:** `govje_parking`
- **Title:** GOVJE Parking
- **Class prefix:** `GOVJEParking`

**When creating new files:**

- Use the domain `govje_parking` for all DOMAIN references
- Prefix all integration-specific classes with `GOVJEParking`
- Use "GOVJE Parking" as the display title
...
- Inherit from platform base + `GOVJEParkingEntity`
...
- `tests/` mirrors `custom_components/govje_parking/` structure
...
- Any rename of `govje_parking` → `ha_integration_domain`
- Any rename of `GOVJEParking` → `IntegrationBlueprint`
- Any rename of `GOVJE Parking` → `Integration Blueprint`
...
Focus only on **genuine upstream improvements**: new scripts, new config files, bug fixes, structural changes, and updates to template-managed AI guidance files.
...
2. Identify the **genuine changes** to keep (new files, real fixes, structural improvements, and template-managed AI guidance updates)
3. Identify the **noise** to revert (placeholder substitutions outside template-managed AI guidance files, devcontainer personal config changes, repo URL changes, and bootstrap workaround regressions listed above)
