...
- Files: `snake_case.py`
- Classes: `PascalCase` prefixed with `GOVJEParking`
...
- Inherit from both platform entity and `GOVJEParkingEntity` (order matters)
...
from custom_components.govje_parking.sensor import async_setup_entry
...
**Config entry data:** `entry_data: GOVJEParkingData = hass.data[DOMAIN][entry.entry_id]`
