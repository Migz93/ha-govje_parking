...
**Directory:** `custom_components/govje_parking/[platform]/`
...
```python
"""[Platform] platform for GOVJE Parking."""
...
from .entity import GOVJEParkingEntity
from .[entity_file] import GOVJEParking[EntityName]
...
async def async_setup_entry(
    ...
    coordinator: GOVJEParkingDataUpdateCoordinator = hass.data[DOMAIN][
        entry.entry_id
    ]

    async_add_entities(
        [
            GOVJEParking[EntityName](coordinator, entry),
...
```python
"""[Entity description] for GOVJE Parking."""
...
Add platform to `custom_components/govje_parking/manifest.json`:
...
- [ ] Entity class inherits from both `GOVJEParkingEntity` and platform class
...
- **Base entity:** `GOVJEParkingEntity` in `entity/base.py`
- **Coordinator:** `GOVJEParkingDataUpdateCoordinator`
